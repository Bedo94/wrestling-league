...
"""
    ...
    with st.expander("Interpretazione intuitiva dei parametri"):
        st.markdown(
            """
### Esperienza pratica
- `years_component` misura l'anzianita di pratica
...
- anche questa componente e limitata da `medals_cap`
...
"""
    ...
    with st.expander("Parametri valutazione livello", expanded=False):
        with st.form("level_evaluation_form"):
            ...
            save_clicked = col1.form_submit_button(
                "Salva e aggiorna valutazione livello"
            )
            reset_clicked = col2.form_submit_button("Ripristina default valutazione livello")

    if save_clicked:
        save_formula_group_draft("level_evaluation", level_inputs)
        set_flash_message(
            "success",
            "Parametri valutazione livello applicati. Anteprima aggiornata.",
            target="level_evaluation",
        )
        st.rerun()

    if reset_clicked:
        reset_formula_group_draft("level_evaluation")
        queue_group_reset("level_evaluation", "level_eval")
        set_flash_message(
            "warning",
            "Parametri valutazione livello della bozza ripristinati ai valori di default.",
            target="level_evaluation",
        )
        st.rerun()
